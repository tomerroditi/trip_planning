...
import type { Accommodation, TripState } from "../types";
import { C, FREDOKA, fmtMoney, statusStyle } from "../theme";

type Filter = "all" | "Booked" | "Pending" | "To book";

export function Stays({ state }: { state: TripState }) {
  ...
  return (
    <div className="fade-up" style={{ height: "100%", overflowY: "auto", padding: "28px 30px 50px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 16, marginTop: 22 }}>
        {visible.map((st) => (
          <StayCard key={st.id} st={st} segName={(st.segment_id && segById.get(st.segment_id)?.name) || ""} segColor={(st.segment_id && segById.get(st.segment_id)?.color) || C.greenMid} currency={state.trip.currency || "NZD"} />
        ))}
...
function StayCard({ st, segName, segColor, currency }: { st: Accommodation; segName: string; segColor: string; currency: string }) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.borderSoft}`, borderRadius: 18, padding: "19px 21px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".02em", color: ss.fg, background: ss.bg, padding: "5px 12px", borderRadius: 20, whiteSpace: "nowrap", flexShrink: 0 }}>{st.status}</span>
      </div>
